*此项目应当在Python 3.xx下执行*
# 这里将会从头到尾阐述整个项目的执行方式
目前仅针对《宝可梦：黑/白》的文本汉化进行实际操作。

项目中存在仅适用于《宝可梦：黑/白》日版的硬编码，若欲将项目应用于其他游戏，则需要修改这些部分。
# 1.准备工作
## 从ROM中导出Narc
...
拆文本→翻译（修改）文本→做码表→改字库→文本和字库都导入Narc分块→打包生成New_Narc→用tinke将New_Narc对位导入
...
以上面的翻译为例：“感觉真有气氛啊。`<PAGE>`”按A，对话框的两行字直接换成
...
## 文本导入打包流程
完成文本修改（翻译）后，把修改（翻译）后的文本组合恢复原本的格式，导入Narc中打包。
### 组合文本
...
```
组合后的文本就在新创建的B(CH)3_extr/目录下，并且已经创建好了待写入的对应文本块。
...
a023-2_Total.txt，
这3个码表的内容应当完全相同。
>> 确认无误后，请任选一个码表作为原始码表（输入0，1，2中的任意一个数字）：0

>> 翻译后的文本*.txt放在多个文件夹中吗？输入Y为是，N为否：Y

>> 请输入第1个路径（输入“END”或按ENTER结束）：./B(CH)2_extr/

>> 请输入第2个路径（输入“END”或按ENTER结束）：./B(CH)3_extr/

>> 请输入第3个路径（输入“END”或按ENTER结束）：\n

已在程式脚本同目录下生成了名为CHS.TBL的中文编码表，专用于制作中文字库。
>> 确认无误后，请按任意键继续后续操作：\n

预替换的第1个字符为编号603的['603', '一']，开始替换。
……
