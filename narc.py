...
class BTNF:
    ...
    def toString(self):
        ret = b"BTNF"
        ret += pack("IIHH", self.header[0], self.header[1], self.header[2], self.header[3])
        return ret
class GMIF:
    ...
    def toString(self):
        ...
        for f in self.files:
            ...
            while l%4:
                l += 1
                ret += b"\xFF"
        self.size = len(ret) + 8#应包括header的8字节
        return b"GMIF"+pack("I", self.size)+ret
class NARC:
    def __init__(self, rawdata):
        if len(rawdata)>0:
            self.magic = rawdata[:4]
    ...
    def toString(self):
        ret = b"NARC"
        ret += pack("IIHH", self.header[0], self.header[1], self.header[2], self.header[3]) + self.btaf.toString(self.gmif)
